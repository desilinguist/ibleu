...
function tokenize(sentence, preserve_case) {
    ...
    skippedRegex = new RegExp("<skipped>", "g");
...
function scoreSegment(docid, segnum, tstSegment, refSegments, docBleuObj, sContainer) {
...
// A function that scores the document and accumulates
// the stats into the bleuObject
function scoreDocument(docid, tstDoc, refDocs, sysBleuObj, sContainer) {

    // create a document level bleuScoreObject for this document
    ...
    for (var segnum=0; segnum<tstDoc.length; segnum++) {
        ...
        scoreSegment(docid, segnum, tstSeg, refSegs, docBleuObj, sContainer);
    }

    // compute the document score and save it
    docscore = docBleuObj.computeBLEU();
    sContainer.documentScores[docid] = docscore;

    // update the sysBleuObj with the docBleuObj and nullify the latter
    sysBleuObj.update(docBleuObj);
    docBleuObj = null;
}
